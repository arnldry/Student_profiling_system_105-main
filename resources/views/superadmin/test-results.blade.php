                ...
                <div class="pb-20">
                    <table class="data-table table stripe hover nowrap">
                        ...
                        <tbody>
                            ...
                            <td>
                                    @php
                                        $info = \App\Models\AdditionalInformation::where('learner_id', $student['id'])->first();
                                    @endphp
                                    {{ $info ? $info->lrn : '-' }}
                                </td>
                                <td>{{ $student['name'] }}</td>
                                ...
                                <td>
                                    @if($student['has_riasec'])
                                        <a href="{{ route('superadmin.student-riasec', $student['id']) }}" class="btn btn-sm btn-info">View RIASEC</a>
                                    @endif
                                    @if($student['has_life_values'])
                                        <a href="{{ route('superadmin.student-life-values', $student['id']) }}" class="btn btn-sm btn-success">View Life Values</a>
                                    @endif
                                </td>
                            </tr>
                            @empty
